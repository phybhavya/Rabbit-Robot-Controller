...
Serial.println(but_values.buton);
 esp_err_t result = esp_now_send(peer_addr, (uint8_t *) &but_values, sizeof(but_values));
  ...
  void joystick()
    {Serial.println("reached0");
  const uint8_t *peer_addr = slave.peer_addr;

  joyvalx = analogRead(36)-3000;  
  Serial.println(joyvalx);

  joyvalx = map(joyvalx, -4095, 4095, -100, 100);
  joyvaly = analogRead(39)-2800;
   Serial.println(joyvaly);
  joyvaly = map(joyvaly, -4095, 4095, -100, 100);

  float ang = atan(joyvalx/joyvaly);
  Serial.println(ang);
//  if(ang<=0.5 || ang>=-0.5){
//    vel_x = 0;
//    vel_y = 0;
