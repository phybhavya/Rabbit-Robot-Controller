...
int i, j, k;
int buttons[6] = {25,33,32,26,34,27};
  ...
  if (slave.channel == CHANNEL) { // check if slave channel is defined
    ...
    if (isPaired) {
      ...
      sendControls();
       ...
       //Serial.println("Joystick mode is on");
       delay(100);
     joystick();}
    } else {
      // slave pair failed
...
Serial.println(but_values.buton);
 esp_err_t result = esp_now_send(peer_addr, (uint8_t *) &but_values, sizeof(but_values));
  ...
  x = map(jx, 0, 4095, -250, 250);//+3*(100-abs(x))/100;
  y = map(jy, 0, 4095, -250, 250);//-1*(100-abs(y))/100;
 ...
 double dist = sqrt(x * x + y * y);
  ...
  if (ang >= 350 && ang <= 370){ang = 360;}
  if(ang>=170 && ang <= 200){ ang =180;}
  if(ang>=260 && ang<=280) {ang = 270;}
  if(ang >=360){
    ang = ang -360;
  }
  Serial.print("Angle : ");
